...
- 任务工作流：可视化编排流程步骤与动态表单项（9 种字段类型、默认值预填），支持流程预览与一键复制
- 平台配置：集中管理系统名、页脚版权、水印开关、功能开关等全局参数，保存即全站生效
- 公告管理：发布/编辑/删除公告，支持置顶、类型与草稿/发布状态，控制台展示最新公告
- 首页幻灯片：管理网站首页轮播图，图片上传、排序、启停与跳转链接
- 控制台：渐变欢迎横幅、统计卡（渐变图标 + SVG 迷你趋势图）、部门分布、系统信息、最新公告
